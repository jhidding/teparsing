...
namespace parser
{
    ...
    class all_t
    {
        ...
        using function_type = typename function_traits<Functor>::function_type;
        ...
        private:
            ...
            bool fmap_all(
                    Iterator &i, Range const &r,
                    std::index_sequence<I...> seq, result_type *result, Inherit* st) const
            {
                result_tuple tmp;
                Iterator const first = i;
                if (all_parsers<Iterator, Range, Inherit, result_tuple, I...>(i, r, st, tmp, I...)) {
                    if (result != nullptr) {
                        call_all<function_type, Inherit> call_f(f);
                        try {
                            call_f.template all<result_type, result_tuple, I...>(result, tmp, st, I...);
        ...
        public:
            ...
            {
                if (std::tuple_size<parser_tuple>::value == 1)
                    return format_name(std::get<0>(ps), rank, defs);
